...
import { confirmSubscription, getSubscriptions } from "@/server/payment";
...
const Page = async ({ searchParams }: Props) => {
  ...
  if (checkout_id && success && invoice && currentUser) {
    const paidCheckout = await confirmSubscription({
      checkout_id,
      invoice,
  ...
  const subscriped = await getSubscriptions({ user_id: currentUser.id });
              ...
              <div className="mt-2">
                <ul className="grid grid-cols-auto gap-2">
                  {Eventsdata.map((service, id) => {
                    return (
                      <EventCard
                        subscriped={subscriped.map((s) => s.user_id)}
                        key={id}
                        event={service}
                      />
                    );
